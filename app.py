...
paul_graham_essay = 'D:/test_local/startupideas.txt'
with open(paul_graham_essay, 'r') as file:
    essay = file.read()

local_model = "D:/Llama_local/llama.cpp/models/Llama-3.2-1B-Instruct-Q6_K.gguf"
...
llama_model = Llama(model_path=local_model)
llama_summerize_model = LlamaCpp(model_path=summerize_model)
...
class PromptResponseSplit(BaseModel):
    text: str
...
async def split_text():
    try:
        ...
        summary_chain = load_summarize_chain(llm = llama_summerize_model, chain_type='map_reduce')
...
@app.post("/summarize", response_model=BotResponse)
async def summarize_text(prompt_request: PromptRequest):
    try:
        
        
        # Extract the generated response
        bot_answer = summerize_user_prompt(prompt_request.user_prompt)
        
        
        
        print("Token pre: ", len(tokenizer.encode(prompt_request.user_prompt, False)))
        print("Token post: ", len(tokenizer.encode(bot_answer, False)))
